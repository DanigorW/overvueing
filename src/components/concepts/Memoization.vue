...
In simaple words the fibonacci sequence is the sum of the last two numbers, the algorithm stops when the result reached to the n number.
To solve the fibonacci algorithm we will use recursion (explained above).
The normal solution is very slow, if we provide the algorithm n = 1000 the time it will take to calculate the fibonacci sequence result can crush your browser.
Why? because it's calculate the same numbers again and again a LOT of times.
Memoization function will store the result of the first calculation and if the calculation already exists it will immediately provide the result.
           ...
           "
            codeblock="//normal version of fibonacci algorithm (slow)                        
const fibonacci = (n) => {
    if (n <= 2) {
        return 1
    } else {
...
//memoize version of fibonacci algorithm (fast)
